  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#bbbbbb] dark:bg-[#111111] p-6 rounded-2xl border border-slate-300 dark:border-[#2a2a2a] relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5"><ICONS.Banknote className="w-24 h-24" /></div>
          <div className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">NIL Collective</div>
        ...
        <div className="bg-[#bbbbbb] dark:bg-[#111111] p-6 rounded-2xl border border-slate-300 dark:border-[#2a2a2a] relative overflow-hidden">
          ...
          <div className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">Rev Share Cap Space</div>
          <div className={`text-3xl font-black mb-2 ${capSpace > 0 ? 'text-slate-900 dark:text-white' : 'text-red-500'}`}>{formatMoney(capSpace)}</div>
        ...
        <div className="bg-[#bbbbbb] dark:bg-[#111111] p-6 rounded-2xl border border-slate-300 dark:border-[#2a2a2a] relative overflow-hidden">
          ...
          <div className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">Marketing Budget</div>
      ...
      <div className="bg-[#bbbbbb] dark:bg-[#111111] p-8 rounded-2xl border border-slate-300 dark:border-[#2a2a2a]">
        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6">Payroll Management</h3>
        
        <div className="space-y-6">
           <div>
              <div className="flex justify-between mb-2">
                 <label className="text-sm font-bold text-slate-700 dark:text-slate-300">Revenue Share Allocation (Current Roster)</label>
                 <span className="text-emerald-400 font-mono">{formatMoney(team.financials.revenueShareAllocated)}</span>
              </div>
              <div className="w-full h-4 bg-slate-50 dark:bg-black rounded-full overflow-hidden">
      ...
      <div className="bg-[#bbbbbb] dark:bg-[#111111] p-8 rounded-2xl border border-slate-300 dark:border-[#2a2a2a]">
         <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-4">Top Earners (Projected)</h3>
         <div className="space-y-3">
            {team.roster.sort((a,b) => b.marketValue - a.marketValue).slice(0, 5).map(player => (
               <div key={player.id} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-black/50 rounded-lg">
                  <div className="flex items-center gap-3">
                     <span className="bg-slate-200 dark:bg-[#1a1a1a] text-slate-900 dark:text-white text-xs font-bold px-2 py-1 rounded">{player.position}</span>
